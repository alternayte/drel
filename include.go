...
func (q *IncludableQuery[T]) Find(ctx context.Context, id any) (*T, error) {
	entity, err := q.builder.Where(newComparison(q.repo.meta.PKColumns[0], ast.OpEq, id)).First(ctx)
	if err != nil {
		return nil, err
	}
	...
	for i := 0; i < len(values); i += includeBatchSize {
		...
		if inc.limit != nil && len(batch) > 1 {
			...
			if len(partitionOrder) == 0 {
				partitionOrder = []ast.OrderByExpr{{Column: meta.PKColumns[0], Direction: ast.Asc}}
			}
			node.OrderBy = nil
			node.Limit = nil
