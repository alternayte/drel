...
import (
	...
	orders "github.com/alternayte/drel/examples/composite-keys/orders"
)

type DB struct {
	*drel.Engine
	OrderLines *orders.OrderLineRepository
}
...
func Open(dsn string, opts ...drel.Option) (*DB, error) {
	...
	return &DB{
		Engine:     engine,
		OrderLines: &orders.OrderLineRepository{Repository: drel.NewRepository(engine, orders.OrderLineMeta)},
	}, nil
}
...
type TxRepos struct {
	OrderLines *orders.TxOrderLineRepository
}
...
func (db *DB) Tx(ctx context.Context) TxRepos {
	...
	return TxRepos{
		OrderLines: &orders.TxOrderLineRepository{TxRepository: drel.NewTxRepository(tx, orders.OrderLineMeta)},
	}
}
